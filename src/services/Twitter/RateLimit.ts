...
class RateLimit {
  ...
  async setLimit(
    ...
    try {
      ...
      if (typeof isAlreadyOnDatabase !== 'string') {
        ...
        logger.info(
          `API rate limit updated on database at RateLimit.setLimit.try`
        );
  ...
  async recalibrate(endpoint: string): Promise<void> {
    try {
      ...
      const { remaining, reset } = resources.statuses['/statuses/retweets/:id'];
